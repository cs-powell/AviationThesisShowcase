...
def log(cogModel, file,timeElapsed,cycleLength): # Get and format data for logging in output file
    ...
    if(timeElapsed > cycleLength):
        sources = [latitudeDREF, longitudeDREF,DREF.ALTITUDE_AGL, pitchDREF,rollDREF,groundspeedDREF]
        data = cogModel.client.getDREFs(sources)
        mainString = []
        precision = 6
        mainString.append(str(round(timeElapsed,precision)))
        mainString.append(",")
        for i in range(0,sources.__len__()):
            value = data[i][0]
            match sources[i]:
                case DREF.ALTITUDE_AGL:
                    value *= 3.28084
                case DREF.GROUND_SPEED:
                    value *= 1.94384
            mainString.append(str(round(value,precision)))
            mainString.append(",")
...
def runExperiment(title,setupArray,allowPrinting,isNewExperiment,file, stop_event: threading.Event):
    ...
    while(timeElapsed <= timeoutLimit and experimentInProgress and (not stop_event.is_set())):
        try:
            with xpc.XPlaneConnect() as client:
                ...
                while(experimentInProgress and (not stop_event.is_set())):
                    ...
                    if(elapsed > currentDelay):
                        cogModel.update_aircraft_state(manualControl) 
            ...
            "Time Elapsed: -----> {c} \n Exception Message:\n{d}".format(a= startTime, b=endTime,c=timeElapsed,d=str(e))
